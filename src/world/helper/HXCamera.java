...
private double updateHertz; // is called often so will keep local reference
	 ...
	 */
	public HXCamera(HXWorldPanel worldParent, double updateHertz) {
	...
	/**
	 * Called only by HXWorldPanel mouseDragged
	 */
	...
	/**
	 * Called only by the updateWorld method in parent panel. Determinde by HXClock
	 * <p>
	...
	private void borderCheck() {
		...
		if (parentPanel.getWidth() >= (leftBorder + rightBorder + (CAM_VIEW_BORDER*zoom) * 2)) {
			camera_pos_x = (int) (parentPanel.getWorld().getWidth()/2 * zoom);
		} else if (camera_pos_x < leftBorder) {
			camera_pos_x = (int) leftBorder;
		} else if (camera_pos_x > rightBorder) {
			camera_pos_x = (int) rightBorder;
		}
		if (parentPanel.getHeight() >= (topBorder + botBorder) + (CAM_VIEW_BORDER*zoom) * 2) {
			camera_pos_y = (int) (parentPanel.getWorld().getHeight()/2 * zoom);
		} else if (camera_pos_y < topBorder) {
